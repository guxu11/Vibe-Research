#!/bin/bash
#SBATCH --job-name=ollama_keyfact_extract
#SBATCH --output=ollama_keyfact_extract_%A_%a.log  # 让每个任务有单独的日志
#SBATCH --gres=gpu:1
...
python3 -u keyfact_alignment.py "${SLURM_ARRAY_TASK_ID}" "${OLLAMA_HOST}"
