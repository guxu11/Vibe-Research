...
def split_sentence_and_fact_error():
    from constants import SENTENCE_DIR, OLLAMA_FACTERROR_DIR, TEXT_CATEGORIES
    for t in TEXT_CATEGORIES:
        print(t)
        from_folder = os.path.join(OLLAMA_FACTERROR_DIR, t)
        to_folder = os.path.join(SENTENCE_DIR, t)
        os.makedirs(to_folder, exist_ok=True)
        files = os.listdir(from_folder)
        files.sort()
        for from_file in files:
            from_file_path = os.path.join(from_folder, from_file)
            with open(from_file_path, "r") as f:
                content = f.read()
            content_json = json.loads(content)
            to_dict = {'raw_text': content_json['raw_text']}
            for k in content_json:
                if k == 'fact_checking_status' or k == 'raw_text':
                    continue
                to_dict[k] = content_json[k]['sentences']
            to_file_path = os.path.join(to_folder, from_file)
